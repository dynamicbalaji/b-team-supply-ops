...
export default function Nav({ scenario, onScenarioChange, onStartScenario, onReset }) {
  ...
  useEffect(() => {
    if (!API_URL) {
      setBackendStatus('manual')
      setStatusDetail('VITE_API_URL not set')
      return
    }
  ...
  return (
    <nav className="nav">
      ...
      <div className="nav-r">
        ...
        <div
          title={`API: ${API_URL || '(not set)'} — ${statusDetail}`}
          style={{
        ...
        <button className="nbtn" ref={btnRef} onClick={() => setDropOpen(v => !v)}>
          ⚙ DEMO CTRL
        </button>
      ...
      <div className={`drop${dropOpen ? ' open' : ''}`} ref={dropRef}>
        ...
        <div style={{
          ...
          color: statusColor, lineHeight: '1.5',
        }}>
          <div>{isLive ? '● Live backend connected' : isChecking ? '○ Connecting…' : '● Manual mode (no backend)'}</div>
          {API_URL
            ? <div style={{ color: '#556', marginTop: '2px' }}>{API_URL}</div>
            : <div style={{ color: '#ff3b5c', marginTop: '2px' }}>⚠ VITE_API_URL not configured</div>}
          {!isLive && !isChecking && statusDetail && (
            <div style={{ color: '#556', marginTop: '2px', wordBreak: 'break-all' }}>{statusDetail}</div>
          )}
        </div>
